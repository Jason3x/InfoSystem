...
printf "\e[?25l" > "$CURR_TTY" # Masquer le curseur
dialog --clear
export TERM=linux
...
ExitMenu() {
    printf "\033c" > "$CURR_TTY"
    printf "\e[?25h" > "$CURR_TTY" # Réafficher le curseur
...
# --- Mise à jour de la base panels depuis GitHub ---
UpdatePanelsDB() {
...
DetectPanel() {
    ...
    local DTB_FILES=$(ls /boot/*.dtb 2>/dev/null)
    for DTB_FILE in $DTB_FILES; do
        local HASH=$(md5sum "$DTB_FILE" | awk '{print $1}')
